...
def handle_encryption(app):
    ...
    is_rsa = "rsa" in algo_name.lower()

    # Pentru RSA direct, fișierul trebuie să fie mic.
    # Folosim 190 bytes ca să meargă și cu Py. Crypt. + OAEP SHA-256.
    if is_rsa and input_bytes > 190:
        messagebox.showwarning(
            "Eroare RSA",
            "Pentru RSA direct, fișierul trebuie să fie sub 190 bytes. "
            "Alege un fișier .txt foarte mic!"
        )
        return
    ...
    if mode == "auto":
        if is_rsa:
            if selected_fw == "OpenSSL":
                ...
                if priv_res.returncode != 0:
                    messagebox.showerror("Eroare OpenSSL", priv_res.stderr)
                    return
                ...
                if pub_res.returncode != 0:
                    messagebox.showerror("Eroare OpenSSL", pub_res.stderr)
                    return
                ...
                # Pentru Py. Crypt. generăm cheia RSA în blocul de criptare,
                # pentru că avem nevoie direct de obiectul private_key_obj.
                pass
        ...
        if is_rsa:
            messagebox.showwarning(
                "Info",
                "Introducerea manuala pentru RSA nu este suportata momentan. "
                "Foloseste modul Auto sau Din baza de date."
            )
            return
        ...
        try:
            ...
            if is_rsa:
                ...
                if mode == "auto":
                    ...
                    messagebox.showwarning(
                        "Eroare",
                        "Pentru RSA cu Py. Crypt. folosește Auto sau Din baza de date."
                    )
                    return
